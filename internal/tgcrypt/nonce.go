...
// TODO: resolve faketls discrepency
var WrongNonceStarters = [...][]byte{
	...
	// {0x16, 0x03, 0x01, 0x02}, //FakeTLS for old hard-coded ls length
	{0xdd, 0xdd, 0xdd, 0xdd}, // padded intermediate header
	{0xee, 0xee, 0xee, 0xee}, // intermediate header
}

var unofficialWrongNonceStarters = [...][]byte{
	{0x16, 0x03, 0x01}, // FakeTLS not in official docs
}
...
func unofficialWrongNonce(nonce *Nonce) bool {
	for _, s := range unofficialWrongNonceStarters {
		if bytes.Equal(nonce[:len(s)], s) {
			return true
		}
	}
...
func genNonce() (packet Nonce) {
	...
	for {
		...
		if IsWrongNonce(&packet) {
			continue
		} else if unofficialWrongNonce(&packet) {
			continue
		}
		return
	}
