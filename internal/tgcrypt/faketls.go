...
var FakeTlsStart = [...]byte{
	0x16, // handhake record
	0x03, // protocol version 3.1
	0x01,
}

const FakeTlsMinHandshakeLen = 512
...
func FakeTlsCtxFromTlsHeader(header []byte, secret *Secret) (c *FakeTlsCtx, err error) {
	digest := header[11 : 11+32]
	msg := make([]byte, len(header))
	copy(msg, header[:])
	for i := 11; i < 11+32; i++ {
		msg[i] = 0
	}
